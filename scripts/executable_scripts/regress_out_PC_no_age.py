...
def unit_regressor_disease_specific(gene,y_data,expression_PC):
    ...
    model = OLS.from_formula(formula=f'{gene} ~ {" + ".join(expression_pcs)} + C(Sex) + C(Disease_class)',data=df)
    result = model.fit()
...
if __name__ == '__main__':
    ...
    for gene in gene_cols:
        delaycompute=delayed(unit_regressor)(gene,
    ...
    repartitioned_collector.to_parquet(args.out_parquet_dir,compression='snappy',write_index=False)
